...
function handleRemoveProject(e: Event, path: string) {
  e.stopPropagation()
  if (confirm('Remove this project from Dashboard? (Data will not be deleted)')) {
    store.removeProjectFromRegistry(path)
  }
}
</script>
...
<template>
  <div class="h-10 flex items-center gap-2 overflow-x-auto px-4 border-b border-sci-border bg-sci-panel flex-shrink-0">
    <button
      ...
      class="group flex-shrink-0 flex items-center gap-2 px-3 py-1.5 text-xs font-mono border-t border-x rounded-t-md transition-all whitespace-nowrap relative top-[1px]"
      :class="[
    ...
    >
      ...
      <!-- Remove Button -->
      <button
        @click="handleRemoveProject($event, proj.path)"
        class="ml-1 p-0.5 rounded opacity-0 group-hover:opacity-100 hover:bg-red-100 hover:text-red-500 transition-opacity"
