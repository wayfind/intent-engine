...
async fn test_ai_cannot_complete_human_owned_task() -> Result<()> {
    ...
    let task = task_mgr
        .add_task("Human Task".to_string(), None, None, None, None, None) // None = human
        .await?;
...
async fn test_mixed_ownership_in_hierarchy() -> Result<()> {
    ...
    // Create human parent
    let parent = task_mgr
        .add_task("Human Parent".to_string(), None, None, None, None, None)
        .await?;
