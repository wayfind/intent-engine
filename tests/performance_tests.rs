...
async fn test_deep_task_hierarchy() {
    ...
    for i in 0..depth {
        let task = task_mgr
            .add_task(format!("Level {}", i), None, parent_id, None, None, None)
            .await
            .unwrap();
...
async fn test_massive_events() {
    ...
    let task = task_mgr
        .add_task(
            "Task with many events".to_string(),
            None,
            None,
            None,
            None,
...
async fn test_concurrent_task_operations() {
    ...
    for i in 0..100 {
        ...
        let handle = tokio::spawn(async move {
            ...
            for j in 0..10 {
                let task = task_mgr
                    .add_task(format!("Task {}-{}", i, j), None, None, None, None, None)
                    .await
                    .unwrap();
