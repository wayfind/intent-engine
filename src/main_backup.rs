...
use intent_engine::cli_handlers::{
    ...
    handle_guide_command, handle_init_command, handle_logs_command, handle_report_command,
    handle_search_command, handle_session_restore, handle_setup, handle_task_command, read_stdin,
};
use intent_engine::dashboard::pid;
...
/// Determine if Dashboard should be auto-started for this command
fn should_auto_start_dashboard(command: &Commands) -> bool {
    match command {
        // Skip Dashboard commands (already handling Dashboard)
        Commands::Dashboard(_) => false,
        // Skip Init (project not set up yet)
        Commands::Init { .. } => false,
        // Skip Doctor (diagnostic command, shouldn't auto-start)
        Commands::Doctor => false,
        // Auto-start for all other commands
        _ => true,
    }
}

/// Ensure Dashboard is running, starting it in daemon mode if needed
async fn ensure_dashboard_running() -> Result<()> {
    // Check if already running via PID file
    if let Some(existing_pid) = pid::read_pid()? {
        if pid::is_process_running(existing_pid) {
            // Check health via HTTP
            if check_dashboard_health().await {
                tracing::debug!("Dashboard already running (PID: {})", existing_pid);
                return Ok(());
            } else {
                // PID file exists but health check failed - clean up stale PID
                tracing::warn!("Dashboard PID exists but health check failed, cleaning up");
                pid::delete_pid_file()?;
            }
        } else {
    ...
    // Dashboard not running, start it in daemon mode
    tracing::info!("Auto-starting Dashboard in daemon mode...");

    // Ensure project context exists (needed for Dashboard to work)
    if let Err(e) = ProjectContext::load_or_init().await {
        tracing::warn!("Failed to load project context: {}", e);
        return Ok(()); // Don't fail CLI command
    }

    // Start Dashboard via direct daemon command invocation
    let current_exe = std::env::current_exe()
        .map_err(|e| IntentError::InvalidInput(format!("Failed to get current exe: {}", e)))?;

    #[cfg(unix)]
    {
        use std::process::Command;
        Command::new(current_exe)
            .args(&["dashboard", "start", "--daemon"])
            .spawn()
            .map_err(|e| IntentError::OtherError(anyhow::anyhow!("Failed to spawn Dashboard: {}", e)))?;
    }
    ...
    // Wait for Dashboard to become healthy (max 3 seconds)
    for i in 0..6 {
        ...
        if i == 5 {
            tracing::warn!("Dashboard health check timeout after 3 seconds");
        }
    }
...
async fn check_dashboard_health() -> bool {
    let health_url = format!("http://127.0.0.1:{}/api/health", DASHBOARD_PORT);
...
async fn run(cli: &Cli) -> Result<()> {
    match cli.command.clone() {
        ...
        Commands::Guide(guide_cmd) => handle_guide_command(guide_cmd)?,
