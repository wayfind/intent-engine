...
pub enum Commands {
    ...
    /// Show usage guides and best practices
    ///
    /// Provides context-rich guides optimized for AI assistants to learn
    /// Intent-Engine workflows and patterns.
    #[command(subcommand)]
    Guide(GuideCommands),
...
pub enum GuideCommands {
    ...
    ///
    /// Shows how AI assistants should use Intent-Engine for task management.
    /// Covers core workflow, common patterns, and best practices.
    ///
    /// Optimized for Claude Code, but applicable to any AI assistant.
    Ai,

    /// TodoWriter compatibility guide
    ///
    /// Explains how Intent-Engine replaces TodoWriter with enhanced features:
    /// - Batch task creation via `plan` interface
    /// - Status management (todo/doing/done)
    /// - Real-time Dashboard sync
    /// - Event history for decisions and blockers
    TodoWriter,

    /// Core workflow patterns
    ///
    /// Demonstrates essential Intent-Engine workflows:
    /// - Focus-driven single-task execution
...
pub enum DashboardCommands {
    ...
    ///
    /// By default runs in foreground mode and can be stopped with Ctrl+C.
    /// Use --daemon to run in background mode with automatic PID management.
    Start {
